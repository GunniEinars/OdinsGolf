...
class MainActivity : ComponentActivity() {
    ...
    // Holds the system splash (the emblem on white) on screen briefly after the
    // first frame is ready, so the brand registers. Flipped false by a delayed post.
    private var keepSplashOn = true

    override fun onCreate(savedInstanceState: Bundle?) {
        // The OS launch splash is the whole opening: the OdinsGolf emblem, big and
        // circle-filling, on white. No Compose splash afterwards, so no double take.
        val splash = installSplashScreen()
        splash.setKeepOnScreenCondition { keepSplashOn }
        super.onCreate(savedInstanceState)
        window.decorView.postDelayed({ keepSplashOn = false }, 650)
        setContent {
            OdinsGolfTheme {
